 ...
 * 备注：上游用的是我们自己的产品
 */
class New2WinxH5Controller extends PayController
{
    ...
    public function Pay($array)
    {
        ...
        $notifyurl = $this->_site ."Pay_New2WinxH5_notifyurl.html"; //异步通知
        $callbackurl = $this->_site . 'Pay_New2WinxH5_callbackurl.html'; //跳转通知
        $parameter = [
            'code'          => 'New2WinxH5',
            'title'         => 'New2WinxH5',
            'exchange'      => 1, // 金额比例
...
$myKey=$return['signkey']; 		
 ...
 $apiurl = 'http://pdd.wkongpay.com/index/api/order'; // API下单地址
...
//var_dump($data);
    ...
    $url =  'http://pdd.wkongpay.com/index/api/order';
        file_put_contents('Data/pddWxPay2.txt', "zf【" . date('Y-m-d H:i:s') . "】提交1结果：" . var_export($data,true) . "\r\n\r\n", FILE_APPEND);
...
/**
    ...
    public function notifyurl()
    {
        ...
        file_put_contents('Data/pddWX2.txt', "zf【" . date('Y-m-d H:i:s') . "】notifyurl提交1结果：" . $outorderno . "--" . $status . "\r\n\r\n", FILE_APPEND);
        file_put_contents('Data/pddWX2.txt', "zf【" . date('Y-m-d H:i:s') . "】notifyurl提交1结果：" . var_export($_POST,true) .$newsig. "\r\n\r\n", FILE_APPEND);
        if ($status == "CODE_SUCCESS" && $newsig == $_REQUEST["sign"]) {
            $Order = M("Order");
            $pay_status = $Order->where("pay_orderid = '" . $outorderno . "'")->getField("pay_status");
            if ($pay_status == 0) {
                $this->EditMoney($outorderno, 'New2WinxH5', 0);
                file_put_contents('Data/pddWX2.txt', "wx【" . date('Y-m-d H:i:s') . "】notifyurl提交2结果：" . $outorderno . "--" . $status . "\r\n\r\n", FILE_APPEND);
                exit("success");
            }
